...
// reg [9:0] pix_x_reg = 500;
// reg [9:0] pix_y_reg = 400;
// assign     pix_x  =  pix_x_reg;
// assign     pix_y  =  pix_y_reg;

reg  [15:0]  pix_data_reg = 332526;

initial begin
    pix_data = pix_data_reg ;
end


//generate vcd
initial begin
    $dumpfile("vga_ctrl.vcd");           //生成的vcd文件名称
    $dumpvars(0,tb_vga_ctrl);    // tb模块名称 module 的名字
end
initial begin
    sys_rst_n = 0;  #(PERIOD/2) ;
    sys_rst_n = 1;  #(PERIOD/2) ;

    forever #(PERIOD/2)  vga_clk=~vga_clk;
end

initial begin
    forever begin
        #100;
        if ($time >= 8000000)  $finish ;
    end
end
endmodule
